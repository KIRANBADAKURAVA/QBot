...
Always generalize and optimize the query to retrieve only what's asked. If a query cannot be answered with the schema provided, return a SQL comment explaining why (e.g., -- Unable to answer because the table does not include required data).
...
async function getcleanSQL(sql) {
    const reply = await groq.chat.completions.create({
        model: "llama3-70b-8192",
        messages: [
            { role: "system", content: `Rewrite the following SQL query . Your task is to return only the correct and syntactically valid SQL query and dont return any other text` },
...
// }
//func();
export { generateSQL, resetChat, getcleanSQL };